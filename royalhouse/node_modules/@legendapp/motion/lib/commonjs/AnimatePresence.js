...
function AnimatePresence(_ref) {
  ...
  const fr = (0, _tools.useForceRender)();

  const childArr = _react.Children.toArray(children);

  const childrenPrevious = (0, _tools.usePrevious)(childArr); // Map children and previous children to { key: child }

  const childrenByKey = exitableByKey(childArr);
  const childrenByKeyPrevious = (0, _tools.usePrevious)(childrenByKey); // Add newly exited elements to the exiting map

  const exiting = (0, _react.useRef)(new Map());
  ...
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, childrenToRender.map(child => {
    if (child && child.props.exit) {
      const key = child.key;
      const animKeys = Object.keys(child.props.exit); // Remove the child when all exit animations end

      return key && exiting.current.get(key) && animKeys ? /*#__PURE__*/(0, _react.cloneElement)(child, {
