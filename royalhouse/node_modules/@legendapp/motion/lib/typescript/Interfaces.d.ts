import type { ComponentClass, ComponentProps, ComponentType, FunctionComponent, JSXElementConstructor, PropsWithChildren, ReactNode } from 'react';
import type { ImageStyle, LayoutChangeEvent, StyleProp, TextStyle, ViewStyle } from 'react-native';
export declare type ComponentStyle<T extends keyof JSX.IntrinsicElements | JSXElementConstructor<any>> = ComponentProps<T>['style'] extends StyleProp<infer P> ? P : ComponentProps<T>['style'];
export declare type StyledProps<P> = PropsWithChildren<P & {
    className?: string;
...
export declare type EaseFunction = 'linear' | 'easeIn' | 'easeOut' | 'easeInOut' | 'circIn' | 'circOut' | 'circInOut' | 'backIn' | 'backOut' | 'backInOut';
...
export declare type MotionTransition = MotionTransitionTween | MotionTransitionSpring;
export declare type MotionTransitionRecord<T> = {
    [key in keyof T]: MotionTransition;
};
export declare type TransformOrigin = `${number}%` | number | `${number}px`;
